...
import 'package:mobile_app/features/profile/presentation/pages/auto_capture_scanner_page.dart';
import 'package:mobile_app/features/profile/presentation/pages/document_scanner_page.dart';
import 'package:mobile_app/features/profile/presentation/pages/face_scanner_page.dart';
import 'package:mobile_app/features/profile/presentation/pages/scanned_image_review_page.dart';
import 'package:mobile_app/features/profile/presentation/pages/selfie_with_document_scanner_page.dart';
...
Future<File?> scanWithCamera(
  ...
  final path = await navigator.push<String>(
    MaterialPageRoute(builder: (_) => _scannerForType(type, title: title, instruction: instruction)),
  );
...
Future<File?> pickImageWithScanner(
  ...
  if (source == ImageSource.camera) {
    return scanWithCamera(context, type: type, title: title, instruction: instruction);
  }
  return _reviewGalleryImage(Navigator.of(context), () async {
    return ImagePicker().pickImage(
      source: source,
      maxWidth: maxWidth,
      maxHeight: maxHeight,
    );
  }, type: type, title: title, instruction: instruction);
}
...
Future<File?> pickVideo(
  ...
  final picked = await ImagePicker().pickVideo(source: source, maxDuration: maxDuration);
...
Future<MediaPickResult?> pickKycMedia(
  ...
  switch (source) {
    case KycMediaSource.photoCamera:
      final file = await scanWithNavigator(navigator, type: type, title: title, instruction: instruction);
      return file == null ? null : MediaPickResult(file);
    case KycMediaSource.photoGallery:
      final file = await _reviewGalleryImage(
        navigator,
        () => ImagePicker().pickImage(source: ImageSource.gallery, maxWidth: 1200, maxHeight: 800),
        type: type,
        title: title,
        instruction: instruction,
      );
    ...
    case KycMediaSource.videoCamera:
      final video = await pickVideo(ImageSource.camera, maxDuration: maxVideoDuration);
      return video == null ? null : MediaPickResult(video, isVideo: true);
    case KycMediaSource.videoGallery:
      final video = await pickVideo(ImageSource.gallery, maxDuration: maxVideoDuration);
      return video == null ? null : MediaPickResult(video, isVideo: true);
  }
}
...
const List<String> _proofVideoExtensions = ['mp4', 'mov', 'm4v', 'webm', '3gp', 'avi', 'mkv'];
const List<String> _proofImageExtensions = ['jpg', 'jpeg', 'png', 'webp', 'heic'];
...
Future<ProofMediaSelection?> pickProofMedia(
  ...
  switch (source) {
    case ProofSource.photoCamera:
      final file = await scanWithNavigator(navigator, title: title, instruction: instruction);
    ...
    case ProofSource.files:
      final result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: [..._proofImageExtensions, ..._proofVideoExtensions, ..._proofDocumentExtensions],
        allowMultiple: true,
      );
