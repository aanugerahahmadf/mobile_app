...
/// pencarian substring.
String matchOcrToDropdownLabel(
  String category,
  String value,
  List<String> labels,
) {
...
String? _semanticKey(String category, String norm) {
  switch (category) {
    case 'gender':
      ...
      if (norm.contains('PEREMPUAN') ||
          norm.contains('WANITA') ||
          norm.contains('FEMALE') ||
          norm == 'WOMAN') {
        return 'female';
      }
      return null;
    case 'religion':
      ...
      if (norm.contains('KRISTEN') ||
          norm.contains('PROTESTAN') ||
          norm.contains('CHRISTIAN')) {
        return 'christian';
      }
      if (norm.contains('KATOLIK') || norm.contains('CATHOLIC')) {
        return 'catholic';
      }
      if (norm.contains('HINDU')) return 'hindu';
      if (norm.contains('BUDHA') ||
          norm.contains('BUDDHA') ||
          norm.contains('BUDDH')) {
        return 'buddhist';
      }
      if (norm.contains('KONGHUCU') ||
          norm.contains('KHONGHUCU') ||
          norm.contains('CONFUCIAN')) {
        return 'confucian';
      }
      return null;
    case 'marital_status':
      if (norm.contains('BELUM') ||
          norm.contains('LAJANG') ||
          norm.contains('SINGLE') ||
          norm.contains('UNMARRIED') ||
          norm.contains('NOTMARRIED')) {
        return 'single';
      }
      if (norm.contains('CERAI') ||
          norm.contains('DIVORC') ||
          norm.contains('JANDA') ||
          norm.contains('DUDA')) {
        return 'divorced';
      }
      if (norm.contains('KAWIN') ||
          norm.contains('MENIKAH') ||
          norm.contains('MARRIED')) {
        return 'married';
      }
      return null;
    case 'occupation':
      if (norm.contains('PELAJAR') ||
          norm.contains('MAHASISWA') ||
          norm.contains('STUDENT')) {
        return 'student';
      }
      if (norm.contains('RUMAH') || norm.contains('HOUSEWIFE') || norm == 'IRT') {
        return 'housewife';
      }
      if (norm.contains('WIRASWASTA') ||
          norm.contains('WIRAUSAHA') ||
          norm.contains('PENGUSAHA') ||
          norm.contains('ENTREPRENEUR') ||
          norm.contains('BUSINESS')) {
        return 'entrepreneur';
      }
      if (norm.contains('PROFESIONAL') ||
          norm.contains('PROFESSIONAL') ||
          norm.contains('DOKTER') ||
          norm.contains('AKUNTAN') ||
          norm.contains('LAWYER')) {
        return 'professional';
      }
      if (norm.contains('KARYAWAN') ||
          norm.contains('PEGAWAI') ||
          norm.contains('EMPLOYEE') ||
          norm.contains('BURUH')) {
        return 'employee';
      }
      return null;
