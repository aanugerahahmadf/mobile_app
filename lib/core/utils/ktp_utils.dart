...
Future<String> extractKtpNumberFromKtp(File imageFile) async {
  try {
    ...
    final possibleMatch = possibleKtpRegex.firstMatch(text.replaceAll(RegExp(r'\s+'), ''));
...
KtpOcrData _parseKtpText(String text) {
  ...
  if (ttl.isNotEmpty) {
    ...
    if (m != null) {
      ...
      if (day > 0 && month > 0 && day <= 31 && month <= 12) {
        birthDate = '${day.toString().padLeft(2, '0')}/${month.toString().padLeft(2, '0')}/$year';
        birthPlace = ttl.substring(0, m.start).replaceAll(RegExp(r'[,\s]+$'), '').trim();
      } else {
        birthPlace = ttl;
      }
    } else {
  ...
  final gender = after(indexOf(RegExp(r'Jenis\s*Kelamin', caseSensitive: false))).toUpperCase();
  final bloodType = after(indexOf(RegExp(r'Gol\s*\.?\s*Darah', caseSensitive: false)));
  ...
  final village = after(indexOf(RegExp(r'Kel\s*/\s*Desa', caseSensitive: false)));
  final district = after(indexOf(RegExp(r'Kecamatan', caseSensitive: false)));

  // ── Agama / Status Perkawinan / Pekerjaan ────────────────────────────────
  final religion = after(indexOf(RegExp(r'^Agama', caseSensitive: false))).toUpperCase();
  final maritalStatus = after(indexOf(RegExp(r'Status\s*Perkawinan', caseSensitive: false))).toUpperCase();
  final occupation = after(indexOf(RegExp(r'Pekerjaan', caseSensitive: false))).toUpperCase();

  // ── Kewarganegaraan / Berlaku Hingga ─────────────────────────────────────
  final nationality = after(indexOf(RegExp(r'Kewarganegaraan', caseSensitive: false)));
