...
class _AppLockPageState extends ConsumerState<AppLockPage> with WidgetsBindingObserver {
  ...
  Future<void> _init() async {
    ...
    await syncAppLockFromDb(service, email: ref.read(currentAccountEmailProvider));
  ...
  Future<void> _verifyPin() async {
    final ok = await verifyPin(_pinBuffer, email: ref.read(currentAccountEmailProvider));
    if (!mounted) return;
    if (ok) {
      _goHome();
    } else {
  ...
  Future<void> _forgotPin() async {
    ...
    if (email == null || email.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.emailNotFound)),
        );
      }
      return;
    }
    ...
    await showOtpVerificationSheet(
      ...
      onVerified: () async {
        await resetAppLock(email: email, service: ref.read(appLockServiceProvider));
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.appLockReset)),
        );
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(
                        _faceEnabled ? Icons.screen_lock_portrait : Icons.fingerprint,
                        size: 52,
                        color: Colors.white,
                      ),
                    ),
                    ...
                    if (showPin) ...[
                      ...
                      const SizedBox(height: 8),
                      if (_pinError)
                        Text(l.wrongPin, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text(
                            _faceEnabled ? l.unlockWith.replaceFirst('%s', l.appLockFaceId) : l.unlockWithFingerprint,
