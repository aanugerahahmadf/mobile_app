...
final currentAccountEmailProvider = Provider<String?>((ref) {
  ...
  if (auth is AuthAuthenticated && auth.user.email.isNotEmpty) return auth.user.email;
  return null;
});
...
class AppLockFlags {
  const AppLockFlags({required this.fingerprint, required this.face, required this.pin});
...
Future<AppLockFlags> loadAppLockFlags({String? email}) async {
  ...
  return AppLockFlags(
    fingerprint: prefs.getBool(_scopedKey(_fingerprintUnlockKey, email)) ?? false,
...
Future<void> syncAppLockFromDb(AppLockService service, {String? email}) async {
  try {
    ...
    if (data.containsKey('fingerprint_enabled')) {
      await prefs.setBool(_scopedKey(_fingerprintUnlockKey, email), data['fingerprint_enabled'] as bool);
    }
    if (data.containsKey('face_enabled')) {
      await prefs.setBool(_scopedKey(_faceUnlockKey, email), data['face_enabled'] as bool);
    }
    if (data.containsKey('pin_enabled')) {
      await prefs.setBool(_scopedKey(_pinUnlockEnabledKey, email), data['pin_enabled'] as bool);
    }
  } catch (_) {
...
final fingerprintUnlockProvider = StateNotifierProvider<FingerprintUnlockNotifier, bool>((ref) {
  return FingerprintUnlockNotifier(ref.watch(currentAccountEmailProvider), ref.watch(appLockServiceProvider));
});
...
class FingerprintUnlockNotifier extends StateNotifier<bool> {
  ...
  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    final value = prefs.getBool(_scopedKey(_fingerprintUnlockKey, _email)) ?? false;
...
final faceUnlockProvider = StateNotifierProvider<FaceUnlockNotifier, bool>((ref) {
  return FaceUnlockNotifier(ref.watch(currentAccountEmailProvider), ref.watch(appLockServiceProvider));
});
...
final pinUnlockProvider = StateNotifierProvider<PinUnlockNotifier, bool>((ref) {
  return PinUnlockNotifier(ref.watch(currentAccountEmailProvider), ref.watch(appLockServiceProvider));
});
...
class PinUnlockNotifier extends StateNotifier<bool> {
  ...
  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    final value = prefs.getBool(_scopedKey(_pinUnlockEnabledKey, _email)) ?? false;
    if (value != state) state = value;
  }
...
Future<void> resetAppLock({String? email, AppLockService? service}) async {
  ...
  service?.updateFlags(fingerprintEnabled: false, faceEnabled: false, pinEnabled: false);
}
