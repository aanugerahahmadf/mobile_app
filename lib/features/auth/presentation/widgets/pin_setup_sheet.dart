...
class _PinSetupSheetState extends ConsumerState<_PinSetupSheet> {
  ...
  Future<void> _advance() async {
    final pin = _buffer;
    if (_step == 0) {
      final ok = await verifyPin(pin, email: ref.read(currentAccountEmailProvider));
      if (!mounted) return;
      if (!ok) {
        setState(() {
          _buffer = '';
  ...
  Future<void> _save(String pin) async {
    ...
    // Sync PIN hash to DB so the server can verify for admin reset.
  ...
  Future<void> _confirmDisable() async {
    final ok = await verifyPin(_buffer, email: ref.read(currentAccountEmailProvider));
    if (!mounted) return;
    if (!ok) {
      setState(() {
        _buffer = '';
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Container(
        ...
        padding: EdgeInsets.only(top: 32, bottom: 32 + MediaQuery.of(context).viewInsets.bottom),
        child: SafeArea(
          top: false,
          child: Column(
