...
class CartState {
  ...
  List<Map<String, dynamic>> get selectedItems => items.where((i) => selectedIds['${i['id']}'] == true).toList();
...
class CartNotifier extends StateNotifier<CartState> {
  ...
  Future<void> fetchCart() async {
    ...
    try {
      ...
      state = state.copyWith(loading: false, error: state.items.isEmpty ? (e.error?.toString() ?? AppErrorCodes.failedLoadCart) : null);
    } catch (e) {
      await _loadFromCache();
      state = state.copyWith(loading: false, error: state.items.isEmpty ? e.toString() : null);
    }
  }
  ...
  Future<Map<String, dynamic>?> removeItem(String cartId) async {
    final removedItem = state.items.where((item) => '${item['id']}' == cartId).firstOrNull;
    try {
  ...
  Future<void> restoreItem(Map<String, dynamic> item) async {
    ...
    if (productId != null) {
      await addItem(productId: productId, quantity: item['quantity'] as int? ?? 1);
    } else if (packageId != null) {
      await addItem(packageId: packageId, quantity: item['quantity'] as int? ?? 1);
    }
  }
  ...
  Future<void> saveForLater(String cartId) async {
    ...
    if (id == null) return;
    _ref.read(wishlistProvider.notifier).toggle(
  ...
  void toggleSelectAll() {
    if (state.selectedIds.length == state.items.length && state.items.isNotEmpty) {
  ...
  Future<void> deleteSelected() async {
    ...
    state = state.copyWith(
      items: state.items.where((i) => state.selectedIds['${i['id']}'] != true).toList(),
      selectedIds: {},
    );
    _cacheToDisk();
  }
