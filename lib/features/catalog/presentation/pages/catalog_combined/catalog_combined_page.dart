...
class CatalogCombinedPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CatalogCombinedPage> createState() =>
      _CatalogCombinedPageState();
}
...
class _CatalogCombinedPageState extends ConsumerState<CatalogCombinedPage> {
  ...
  List<_CatalogItem> get _filteredItems {
    ...
    if (_selectedCategoryId != null) {
      items = items
          .where((e) => '${e.data['category_id']}' == _selectedCategoryId)
          .toList();
    }
    ...
    switch (_selectedSort) {
      case 'price_asc':
        items.sort(
          (a, b) => parseDouble(
            a.data['price'],
          ).compareTo(parseDouble(b.data['price'])),
        );
      case 'price_desc':
        items.sort(
          (a, b) => parseDouble(
            b.data['price'],
          ).compareTo(parseDouble(a.data['price'])),
        );
      case 'newest':
        items.sort(
          (a, b) => parseInt(b.data['id']).compareTo(parseInt(a.data['id'])),
        );
      case 'rating_desc':
        items.sort(
          (a, b) => parseDouble(
            b.data['average_rating'],
          ).compareTo(parseDouble(a.data['average_rating'])),
        );
      case 'rating_asc':
        items.sort(
          (a, b) => parseDouble(
            a.data['average_rating'],
          ).compareTo(parseDouble(b.data['average_rating'])),
        );
    }
  ...
  Future<void> _fetchAll() async {
    ...
    try {
      ...
      final packages = _extractList(
        results[0],
      ).map((e) => _CatalogItem(e, 'packages')).toList();
      final products = _extractList(
        results[1],
      ).map((e) => _CatalogItem(e, 'products')).toList();
