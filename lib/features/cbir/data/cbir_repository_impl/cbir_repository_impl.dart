...
import '../models/cbir_result_model/cbir_result_model.dart';
...
class CbirRepositoryImpl implements CbirRepository {
  ...
  Future<List<CbirResultItem>> searchByImage(File imageFile) async {
    ...
    final endpoint = ApiEndpoints.searchImage;
  ...
  Future<List<CbirResultItem>> arithmeticSearch({
    ...
    final arithmeticEndpoint = ApiEndpoints.cbirArithmetic;
    ...
    if (raw is Map && raw['status'] == 'error') {
      throw DioException(
        ...
        response: response,
        message:
            raw['message'] as String? ?? AppErrorCodes.arithmeticSearchFailed,
      );
    }
