...
import '../../../../../core/widgets/credit_card_form/credit_card_form.dart';
...
import '../../../../order/data/order_repository_impl/order_repository_impl.dart';
import '../../../domain/payment_method_info/payment_method_info.dart';
import '../../providers/payment_provider/payment_provider.dart';
import '../../widgets/bank_transfer_detail/bank_transfer_detail.dart';
import '../../widgets/payment_countdown/payment_countdown.dart';
...
class PaymentMethodDetailPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PaymentMethodDetailPage> createState() =>
      _PaymentMethodDetailPageState();
}

class _PaymentMethodDetailPageState
    extends ConsumerState<PaymentMethodDetailPage> {
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      if (ok) {
        ...
        AppSnackBar.show(
          context,
          ref.read(paymentProvider).error ?? l.failedProcessPayment,
  ...
  Future<void> _createVirtualAccount() async {
    ...
    try {
      final va = await ref
          .read(paymentProvider.notifier)
          .createVirtualAccount(widget.orderId);
      if (!mounted) return;
      setState(() => _va = va);
      if (va == null) {
        AppSnackBar.show(
  ...
  Future<void> _payWithGateway() async {
    ...
    try {
      ...
      await context.push(
        '/payment-webview',
        extra: {'url': redirectUrl, 'title': widget.method.name},
      );
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    if (authState is! AuthAuthenticated) {
      return Scaffold(
        ...
        body: const GuestAuthPrompt(icon: Icons.payment_outlined),
      );
    }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: AppSizes.md),
                    if (widget.method.deeplink != null &&
                        widget.method.deeplink!.isNotEmpty) ...[
                      AppButton(
          ...
          child: SizedBox(
            ...
            child: AppButton(
              label: widget.method.gatewayEnabled
                  ? l.payNow
                  : (widget.method.type == 'credit_card'
                        ? l.confirmPayment
                        : l.iHaveTransferred),
              loading: _submitting,
              onPressed: _submitting
                  ? null
                  : (widget.method.gatewayEnabled ? _payWithGateway : _confirm),
            ),
          ),
        ),
      ),
  ...
  Widget _buildHeader(AppLocalizations l) {
    final item =
        _order!['package'] as Map<String, dynamic>? ??
        _order!['product'] as Map<String, dynamic>?;
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(AppSizes.md),
        child: Column(
          ...
          children: [
            ...
            Text(
              '${l.order} #${_order!['order_number'] ?? widget.orderId}',
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    Formatters.currency(
                      Formatters.parsePrice(_order!['total_price']),
                    ),
        ...
        ClipRRect(
          ...
          child: Image.network(
            ...
            errorBuilder: (context, error, stackTrace) =>
                _buildQrisPlaceholder(AppLocalizations.of(context)!),
          ),
        ),
        const SizedBox(height: AppSizes.sm),
        Text(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 2),
                    Text(
                      l.payWithEWallet(widget.method.name),
                      style: AppTextStyles.bodySmall.copyWith(
                        color: AppColors.textSecondary,
                      ),
                    ),
        ...
        children: [
          ...
          if (_qris == null)
            ...
            if (_qris!['qr_expiry'] != null)
              _buildInfoRow(
                l.vaExpiredAt,
                _formatDate(_qris!['qr_expiry'] as String),
              ),
          ],
        ],
      ),
        ...
        children: [
          ...
          if (_va == null)
            ...
            if (_va!['virtual_account_expiry'] != null)
              _buildInfoRow(
                l.vaExpiredAt,
                _formatDate(_va!['virtual_account_expiry'] as String),
              ),
