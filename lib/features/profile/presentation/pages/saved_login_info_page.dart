...
class SavedLoginInfoPage extends StatefulWidget {
  const SavedLoginInfoPage({super.key});

  @override
  State<SavedLoginInfoPage> createState() => _SavedLoginInfoPageState();
}

class _SavedLoginInfoPageState extends State<SavedLoginInfoPage> {
  ...
  Future<void> _fetchStatus() async {
    ...
    try {
      ...
      if (data is Map<String, dynamic> && data['data'] is Map) {
        setState(() {
          _savedLoginEnabled = (data['data'] as Map)['saved_login_enabled'] ?? true;
          _loading = false;
        });
      } else {
  ...
  Future<void> _toggleSavedLogin(bool enabled) async {
    ...
    try {
      await DioClient.instance.post('/security/saved-login/toggle', data: {'enabled': enabled});
                  ...
                  children: [
                        ...
                        child: Column(
                          children: [
                            Icon(
                              ...
                              size: 48,
                              color: _savedLoginEnabled ? AppColors.primaryColor : AppColors.textSecondary,
                            ),
                            const SizedBox(height: AppSizes.sm),
                            Text(
                    ...
                    Card(
                      ...
                      child: SwitchListTile(
                        secondary: Icon(
                          _savedLoginEnabled ? Icons.bookmark : Icons.bookmark_border,
                          color: _savedLoginEnabled ? AppColors.primaryColor : AppColors.textSecondary,
                        ),
                        title: Text(l.saveLoginInfo, style: AppTextStyles.bodyMedium),
                        subtitle: Text(
