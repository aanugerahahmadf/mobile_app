...
import '../../../../auth/presentation/widgets/pin_setup_sheet/pin_setup_sheet.dart';
...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  Future<void> _refreshCacheSize() async {
    final size = await getAppCacheSize();
  ...
  void _runPrivate(VoidCallback action) {
  ...
  Future<void> _editBiometric(BiometricType type, AppLocalizations l) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(success ? l.success : l.failed)));
  }
  ...
  Future<void> _addPin(AppLocalizations l) async {
    ...
    final hasValidPin = await hasValidStoredPin(
  ...
  void _confirmDeleteBiometric(BiometricType type, AppLocalizations l) {
    final label = type == BiometricType.fingerprint
        ? l.biometricFingerprint
        : l.appLockFaceId;
    showDialog<void>(
      context: context,
  ...
  Future<void> _deleteAccount() async {
    ...
    if (ok) {
      ...
      final msg = authState is AuthError
          ? authState.message
          : AppErrorCodes.anErrorOccurred;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(LocalizedError.of(l, msg))));
    }
  }
  ...
  Future<void> _clearCache() async {
    ...
    final bytes = await clearAppCache();
    await _refreshCacheSize();

    if (!mounted) return;
    Navigator.of(context).pop();

    final sizeStr = formatCacheSize(bytes);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(l.cacheClearedBytes(sizeStr))),
    );
  }
        ...
        child: ListView(
          children: [
            ...
            _menuTile(
              ...
              l.notifications,
              () => _runPrivate(() => context.push('/notification-settings')),
            ),
            _menuTile(
              Icons.flag_outlined,
              l.report,
            ...
            Card(
              ...
              child: SwitchListTile(
                ...
                subtitle: Text(
                  isDark ? l.useLightTheme : l.useDarkTheme,
            ...
            Text(
              l.passwordAndSecurity.toUpperCase(),
              style: AppTextStyles.bodySmall,
            ),
            const SizedBox(height: AppSizes.sm),
            _menuTile(
              Icons.security_outlined,
              l.passwordAndSecurity,
              () => _runPrivate(() => context.push('/password-and-security')),
            ),
            const SizedBox(height: AppSizes.lg),
            ...
            _lockMethodCard(
              ...
              onDelete: () =>
                  _confirmDeleteBiometric(BiometricType.fingerprint, l),
            ),
            _lockMethodCard(
            ...
            // ── PENYIMPANAN ──────────────────────────────────────────
            Text(l.clearCache.toUpperCase(), style: AppTextStyles.bodySmall),
            const SizedBox(height: AppSizes.sm),
            Card(
              margin: EdgeInsets.zero,
              shape: RoundedRectangleBorder(
              ...
              child: ListTile(
                ...
                title: Text(l.clearCache, style: AppTextStyles.bodyMedium),
                subtitle: Text(
                  _cacheSize == null
                      ? l.clearCacheDesc
                      : '${l.cacheSize}: ${formatCacheSize(_cacheSize!)}',
                  style: AppTextStyles.bodySmall,
                ),
                trailing: Icon(
                  Icons.chevron_right,
