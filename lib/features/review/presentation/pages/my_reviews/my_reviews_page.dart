...
import '../../providers/my_reviews_provider/my_reviews_provider.dart';
...
class _MyReviewsPageState extends ConsumerState<MyReviewsPage> {
  ...
  Future<void> _deleteReview(Map<String, dynamic> r) async {
    ...
    try {
      ...
      if (mounted) {
        AppSnackBar.show(context, l.reviewDeleted, type: SnackBarType.success);
      }
    } catch (e) {
      if (mounted) {
        AppSnackBar.show(
          context,
          l.failedDeleteReview,
  ...
  Future<void> _editReview(Map<String, dynamic> r) async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => _EditReviewSheet(
        review: r,
        onSave: (data, photoPaths, {removedPhotoUrls}) => ref
  ...
  Widget build(BuildContext context) {
    ...
    if (authState is! AuthAuthenticated) {
      return Scaffold(
        ...
        body: const GuestAuthPrompt(icon: Icons.rate_review_outlined),
      );
    }
  ...
  Widget _buildReviewCard(AppLocalizations l, Map<String, dynamic> r) {
    ...
    final firstMedia = media.isNotEmpty && media[0] is Map
        ? media[0] as Map
        : null;
    final image = firstMedia != null
        ? Formatters.imageUrl(
            (firstMedia['url'] as String?) ??
                (firstMedia['original_url'] as String?),
          )
        : '';
  ...
  Future<void> _openItemReviews(Map<String, dynamic> r) async {
    final package = r['package'] as Map<String, dynamic>?;
    final product = r['product'] as Map<String, dynamic>?;
    final item = package ?? product;
    final id = '${item?['id'] ?? ''}';
    if (id.isEmpty || _navigationBusy || !mounted) return;
...
class _EditReviewSheetState extends State<_EditReviewSheet> {
  ...
  void initState() {
    ...
    _titleController = TextEditingController(
      text: widget.review['title'] as String? ?? '',
    );
  }
  ...
  Widget _buildPhotoPicker(AppLocalizations l, List<String> existingPhotos) {
    ...
    final visibleExisting = existingPhotos
        .where((u) => !_removedExistingUrls.contains(u))
        .toList();
    return Wrap(
      spacing: 6,
      runSpacing: 6,
  ...
  Future<void> _save() async {
    ...
    try {
      await widget.onSave(
        ...
        _newPhotos,
        removedPhotoUrls: _removedExistingUrls.toList(),
      );
      if (mounted) {
      ...
      if (mounted) {
        AppSnackBar.show(
          context,
          '${l.failedUpdateReview}: ${LocalizedError.of(l, e.toString())}',
          type: SnackBarType.error,
        );
      }
    } finally {
