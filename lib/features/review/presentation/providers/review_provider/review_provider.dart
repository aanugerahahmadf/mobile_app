...
import '../../../data/review_repository_impl/review_repository_impl.dart';
...
class ReviewState {
  ...
  ReviewState copyWith({
    ...
    return ReviewState(
      ...
      sort: sort ?? this.sort,
      filterRating: clearFilterRating
          ? null
...
class ReviewNotifier extends StateNotifier<ReviewState> {
  ...
  Future<void> fetchItemReviews({
    ...
    try {
      final result = packageId.isNotEmpty
          ? await _repository.getPackageReviews(
              packageId,
              sort: state.sort,
              rating: state.filterRating,
              withPhoto: state.filterWithPhoto,
            )
          : productId.isNotEmpty
          ? await _repository.getProductReviews(
              productId,
              sort: state.sort,
              rating: state.filterRating,
              withPhoto: state.filterWithPhoto,
  ...
  void setFilterWithPhoto(bool value) =>
      state = state.copyWith(filterWithPhoto: value);

  // ── Helpful vote (optimistic) ───────────────────────────────────────
  Future<void> voteHelpful(int reviewId) async {
    final previous = state.reviews;
    final isCurrentlyVoted =
        state.reviews
          ...
          if ((r['id'] as num?)?.toInt() == reviewId)
            {
              ...
              'helpful_count': currentCount + (isCurrentlyVoted ? -1 : 1),
            }
          else
            r,
      ],
