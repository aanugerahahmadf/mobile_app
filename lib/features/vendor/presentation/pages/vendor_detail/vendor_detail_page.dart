...
import 'package:mobile_app/core/constants/app_sizes/app_sizes.dart';
import 'package:mobile_app/core/widgets/app_filter_widgets/app_filter_widgets.dart';
...
class _VendorDetailPageState extends ConsumerState<VendorDetailPage> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final res = await DioClient.instance.get(
        ApiEndpoints.vendorDetail(widget.id),
      );
      ...
      if (mounted) {
        ...
        final items = <_VendorItem>[
          ...pkgRaw.map(
            (e) => _VendorItem(
              ItemModel.fromJson(e as Map<String, dynamic>),
              'packages',
            ),
          ),
          ...prdRaw.map(
        ...
        for (final vi in items) {
          ...
          if (cat != null) {
            catMap['${cat.id}'] = {'id': '${cat.id}', 'name': cat.name};
          }
        }
  ...
  List<_VendorItem> get _filteredItems {
    ...
    if (_selectedCategoryId != null) {
      items = items
          .where((e) => '${e.data.categoryId}' == _selectedCategoryId)
          .toList();
    }
    switch (_selectedSort) {
      ...
      case 'rating_desc':
        items.sort(
          (a, b) => b.data.averageRating.compareTo(a.data.averageRating),
        );
      case 'rating_asc':
        items.sort(
          (a, b) => a.data.averageRating.compareTo(b.data.averageRating),
        );
    }
    return items;
  }
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => openReportChat(
              ...
              ref,
              category: 'vendor',
              itemName: _vendor?['store_name'] as String? ?? '',
              details: l.reportVendor,
            ),
          ),
        ],
      ),
    ...
    if (items.isEmpty) {
      return SliverToBoxAdapter(
        child: Padding(
          ...
          child: Center(
            child: Text(
              AppLocalizations.of(context)!.noMatchingItems,
              style: TextStyle(color: AppColors.textSecondary),
            ),
          ),
        ),
